...
Research in this field is aimted at letting computational agents draw logic inferences automatically.
%
...
%
Research in this field is aimted at letting sub-symbolic algorithms attain better predictive performances.
%
...
Symbolic (i.e., logic-based) reasoning approaches root back to John McCarthy's work of 1958 \cite{Mccarthy1958}, aimed at developing the idea of formalising the so-called \emph{commonsense reasoning} to build intelligent artefacts---i.e. cmputational or cyber-physical agents endowed with human-like intelligence.
%
...
\begin{equation*}
    \begin{array}{c}
        \phi = (grandparent(X, Y) \leftarrow parent(X, Z), parent(Z, Y))
        \\
        \psi = (grandparent(\functor{abraham}, \functor{jacob}) \leftarrow parent(\functor{abraham}, \functor{isaac}), parent(\functor{isaac}, \functor{jacob}))
    \end{array}
\end{equation*}
%
The two clauses can be made syntactically equal via the substitution $\sigma = \{ X \mapsto \functor{abraham}, Y \mapsto \functor{jacob}, Z \mapsto \functor{isaac} \}$, because $\psi = \phi \cdot \sigma$.
...
%
\begin{equation}
    unifier(\phi, \psi) = \begin{cases}
        \varnothing & \text{if} ~ \phi = \psi = \functor{x}
        \\
        \{ X \mapsto Y \} & \text{if} ~ \phi = X \wedge \psi = Y
        \\
        \bigcup_{i}^{N} unifier(\alpha_i, \alpha'_i) & \text{if} ~ \phi = f(\alpha_1, \ldots, \alpha_N) 
        \\
        & \qquad \wedge\ \psi = f(\alpha'_1, \ldots, \alpha'_N)
        \\
        unifier(\alpha_1, \alpha'_1) \cup unifier(\alpha_2, \alpha'_2)  & \text{if} ~ \phi = \alpha_1 \odot \alpha_2 \wedge \psi = \alpha'_1 \odot \alpha'_2
        \\
        \bot & \text{otherwise}
    \end{cases}
\end{equation}
%
where $\bot$ denotes the lack of any unifier -- capturing the situation where two formul\ae{} cannot be unified -- and $\varnothing$ denotes the emptu substitution -- characterising the situation where two formul\ae{} are identical.
%
...
Inteference rules alone are able to express reasoning, but they are not enough to let computational draw inferences autonomosly.
%
When dealing with inference, computational agents need an inference \emph{procedure}, i.e. an algorithm applying one or more inference rules to a knowledge base, in such a way that counclusions are eventually reached when the algorithim terminates.
...
\begin{inlinelist}
    ...
    \item Clark's proposal of negation as failure \cite{Clark77} as an extension to the SLD  principle,
...
%
The basic inference rule operating in this context is that, in a set of Skolemized formul\ae{} $\mathbf{K}$ including $\phi$ and $\phi$, those formul\ae{} may be reduced $\phi'$ and $\psi'$, under the following condition:
%
\begin{equation*}
    \frac{x \in \phi \quad \lnot y in \psi \quad \sigma = mgu(x, y)}{\phi' = (\phi - x)\cdot\sigma \quad (\psi - \lnot y)\cdot\sigma}[\text{SL}]
\end{equation*}
%
...
%
In other words, if any two formul\ae{} in $\mathbf{K}$ respectively include two literals, $x$ and $y$, which can be unified by $\sigma$ and such that \emph{only} one is negated, then the two literals are eviction from the corresponding formul\ae{}, and the substitution $\sigma$ is be applied to the reminder of those formul\ae{}.
%
...
%
When this is the case, proof by refutation if applied to the set $\gamma \cup \mathbf{K}$: at each step, some literal of $\gamma$ is reduced, along with some other formula in $\textbf{K}$.
%
Upon termination, if all literals of $\gamma$ have been evicted, then $\gamma$ is considered unsatisfiable.
%
Otherwise, the formula $\gamma \cdot \sigma_1 \cdot \sigma_2 \cdots$ -- attained by appling all the MGU computed at each reduction step to the original goal $\gamma$ -- represents a \emph{solution} for $\gamma$, according to $\mathbf{K}$.
%
The whole process subtend the so-called \emph{proof-tree}: at each step of the procedure, several formual\ae{} may be selected from $\mathbf{K}$, therefore the algorithm may follow as many different paths.
%
