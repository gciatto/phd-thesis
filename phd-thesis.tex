...
%
Its instances spread from \emph{deductive databases} \cite{green1968} to \emph{description logics} \cite{baader2002} and \emph{ontologies} \cite{cimiano2006-ontologies}, to name just a few. 
%
...
%
In supervised learning, the learning task consists of finding a way to approximate an unknown relation, given a sampling of it---which constitutes the experience.
%
In unsupervised learning, the learning task consists of finding the best relation for a sample of data -- which constitute the experience --, following a given optimality criterion intensionally describing the relation.
%
...
%
An important trait which significantly affects both the theory and the practice of sub-symbolic supervised learning is that it relies on the assumption that vectors, matrices, or tensors in $\mathcal{X}$ and $\mathcal{Y}$ are of \emph{fixed} size---despite items in $\mathcal{X}$ may have a different size than items in $\mathcal{Y}$.
%
...
To approximate function $\pi^*$, supervised learning leverages upon a learning \emph{algorithm}, which performs approximation by taking into account a number $N$ of \emph{examples} of the form $(\mathbf{x}_i,\mathbf{y}_i)$ such that $\mathbf{x}_i \in X \subset \mathcal{X}$, $\mathbf{y}_i \in Y \subset \mathcal{Y}$, and $|X| \equiv |Y| \equiv N$.
%
...
%
\begin{inlinelist}
    \item an error function $\varepsilon : \mathcal{Y} \times \mathcal{Y} \rightarrow \mathbb{R}_{\geq 0}$ measuring the discrepancy among the expected outputs in $Y$ and the values attained by applying $\hat{\pi}$ to $X$ -- to be minimised --, or dually,
    \item an adherence function $\rho : \mathcal{Y} \times \mathcal{Y} \rightarrow \mathbb{R}_{\leq 1}$, measuring the similarities among the same values---to be maximised.
\end{inlinelist}
%
...
\paragraph{About generalisation: the bias-variance trade-off.}
