...
%
A nice and update survey of ILP is provided by \cite{CropperD2020}.
%
...
%
Roughly speaking, a \emph{general} theory should contain one non-ground clauses, from which the positive examples could be attained via variables substitutions or via deduction.
%
Despite the expected result of any ILP algorithm is certainly a \emph{general} from which all the positive examples, looking for the \emph{most} -- or \emph{least} -- possible general rule is likely a mistake.
%
...
%
There, ILP is conceived as an higher-order logic task where a logic program (the meta-interpreter) dynamically constructs another logic program (the induced theory) via meta-programming.
