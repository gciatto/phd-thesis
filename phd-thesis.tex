...
\sidenote{George Boole, The Laws of Thought. 1854/2003. p. 42}
...
\sidenote{\url{https://en.wikipedia.org/wiki/Peano_axioms}}
...
%
Indeed, as further discussed in \cref{chap:reasoning}, sub-symbolic data processing is commonly tailored on arrays of \emph{fixed} sizes---meaning that the actual values of $d_1, \ldots, d_D$ are chosen at design time and never changed after that.
%
...
An important distinction, when data is represented in the form of numeric arrays, is about whether the representation is \emph{local} or \emph{distributed}.
%
...
%
The single pixel carries very small information when considered alone, whereas groups of contiguous pixel may describe datails which are relevant for image processing. 

\paragraph{Feature Engineering}

Of course, not all data is both rigid and numeric in nature.
%
So, to fit this paradigm, data scientists designed a plethora of conversions methods to transform data from various forms (e.g. possibly non-rigid or non-numeric, when not both) into rigid arrays of numbers.
%
...
\begin{description}
    ...
    \item[categorical] features may be converted into boolean features via the one-hot encoding;
...
\begin{description}
    ...
    \item[apply a domain-chancing transformation] such as the Fourier transform or the wavelet transform;
    \item[sub-sample each variable-size datum] using a fixed-size sampling step; e.g. a sliding window for time series, or neighbourhoods of fixed sizes for graphs;
    \item[exploit \emph{ad-hoc} embeddings] targetting particular sorts of data, such as GNN, Word2Vec, etc.
\end{description}
