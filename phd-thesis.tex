...
\begin{table}
    $$\begin{array}{rcl}
        ...
        \\
        \meta{Boolean} & := & \terminal{\top} \mid \terminal{\bot}
    \end{array}$$
...
%
\begin{itemize}
    \item a number of constant symbols, including:
    %
    \begin{itemize}
        \item a number of \emph{functors}, denoted by monospaced symbols such as $\functor{f}_1, \functor{f}_2, \ldots$,
        \item all real numbers and all possible Boolean values (denoted by $\top$ and $\bot$);
    \end{itemize}  
...
%
Being containers of terms, structures enable the creations of arbitrarily complex data structures combining several entities from the domain of the discourse, and treating them as a whole.
%
...
\end{equation*}
%
Yet, recursive intensional are very expressive and powerful, as they enable the descrition of infinite sets via a finite (and commonly small) amount of formul\ae{}.
...
%
On other words, the particular shape predicates and structures may be chosen in manifold ways, depending on the nature of the data at hand, and on the computations that designers are expecting for that data.

Here we briefly examinate the two extremes a spectrum of possibilities, with the purpose of discussing how each choice in KR may come with both pros and cons, which must therefore be engineered.
...
%
Then we may choose to define each    

\paragraph{Representing tabular data.}

\note{Relational as bidirectional}
\note{Other kinds of logic: description logic, Horn logic}.
