...
% In particular, we consider logical tasks that may benefit from the leverage of sub-symbolic techniques.
%
...
% %
% We consider a theory \emph{fragmented} whenever it is missing some relations or entities attributes.
...
%
%The considered task is very complex from a logical perspective, as it requires a shift from the perspective of rules and knowledge manipulation.
...
%
% Indeed, missing relations usually belong to the same knowledge base of the logic theory.
%
% Therefore, a symbolic approach leveraging only known rules is probably not sufficient to extract missing information from an fragmented theory.
%
...
%
% Additional information may be used  in order to reconstruct the missing facts, requiring a more complex approach and the assumption of the existence of a broader and more complete knoweldge.
...
%
%To reconstruct the missing information, a purely symbolic approach would probably require to resort to a broader knowledge set, which is not straightforward in most cases.
%%
...
%
% The additional entities and rules come from the information of a broader knowledge set---thus the name knowledge inclusion.
%
...
%
% Nevertheless, clauses should be compared depending on their relevance with respect to the original theory.
...
% Given a theory, it is possible to ask for queries resolution through symbolic execution of the theory.
...
%
To this regard, %it represents a relevant task to speed-up query resolution, compressing the pipeline of rules execution.
a number of efficiency tweaks may affect the execution time in the average case.
%
...
%
% In the graph ML community, this problem is known as link prediction---i.e. given a graph, predict which arcs should or will form between vertices which are not linked yet.
%

% Link prediction for knowledge filling is more complex than common link prediction.
%
% Indeed, it requires to predict not only arc existence but also the class of link that should form---i.e. which class of relation involves the entities considered.
%
...
We denote as ``symbolic knowledge injection'' (SKI) the task of letting a sub-symbolic predictor exploit formal, symbolic information to improve its predictive performance (e.g.\ accuracy, f1-measure, learning time) over data.
%
...
%
The GNN-based manipulation of logic theories discussed in \cref{sec:gnn} is an approach of this sort, as well as the many methods for knowledge graph embedding---nicely surveyed in \cite{WangMWG17}.
...
In this work we tackle the problem from another prospective.
%
We discuss how prior \emph{symbolic} knowledge can be \emph{injected} (SKI) into NN, during learning, in order to endow them with the designer's common sense.
%
The injected knowledge may thus encourage the NN to learn desired information, while preventing it from violating the designer's constraints---expressed by symbols.
