...
These algorithms can be categorised in several ways, for instance depending on the supervised learning task they support (classification vs. regression), on when they consume data (lazy vs. eager), or on the underlying strategy adopted for learning (e.g. gradient descent, least squares optimization), etc.

Some learning algorithms target regression problems (e.g. neural networks), whereas others target classification problems (e.g. SVM).
%
Similarly, some target multi-dimensional outputs ($\mathbf{y} \in \mathbb{R}^n$), whereas others target mono-dimensional outputs ($n = 1$).
%
Mono-dimensional regressors are considered as sufficiently general as
%
\begin{inlinelist}
    \item binary classifiers can be treated as mono-dimensional regressors where admissible outputs lay in the interval $[0, 1]$, and
    \item multi-class (resp. multi-dimensional) classifiers (resp. regressors) can be treated as ensembles of multiple binary (resp. regressors)
\end{inlinelist}
...
%
This make the training phase quicker, at the expense of a slower inference phase.
%
...
%
Neural networks are commonly trained via back-propagation \cite{Rumelhart1986} -- a particular case of gradient descent, tailored on NN --, generalised linear models via Gauss' least squares method, decision trees via CART \cite{breiman1984classification}, etc.
%
...
% introduce DT as the most intuitive algorithm and NN as the most flexible.
\sidenote{Riprendi da qui}

\paragraph{Decision Trees}

DT can be considered more interpretable due to their construction: that is, recursively partitioning the input space $\mathcal{X}$ through a number of splits or \emph{decisions} based on the input data $X$, in such a way that the prediction in each partition is constant, and the loss w.r.t.\ $Y$ is low, while keeping the amount of partitions low as well.
%
...
Neural networks are biologically-inspired computational models, made of several elementary units (neurons) interconnected into a \emph{directed-acyclic} graph (DAG) via \emph{weighted} synapses.
%
NN can be \emph{trained} on data via back-propagation \cite{Rumelhart1986} and exploited into both supervised and unsupervised learning tasks such as classification, regression, and anomaly detection.
%
% More precisely, while the training phase sets up synapses weights automatically,
The training phase makes a NN learn from data.
%
Yet, only network synapses weights are modified in this phase, whereas its overall graph structure (topology henceforth) is not allowed to vary.
%
It is rather assumed to be \emph{manually} engineered by data scientists.

Convolutional Neural Networks (CNN) are particular sorts of NN whose topology consists of a cascade of \emph{convolutional} layers.
%
In other words, CNN can learn how to apply a number of convolution operations \cite{o2015introduction} to the data.
%
Convolutions let the network spot relevant features into the input data, at possibly different scales.
%
Thus, CNN are primarily used to solve complex pattern-recognition tasks, such as in computer vision or speech recognition.
%
Yet, how many convolutions a network may learn as well as their interconnections depend on NN topology.
%
Again, this implementation detail requires human intervention.

Traditionally, NN development workflows are deeply influenced by the choices by human experts.
%
Network architectures represent the most relevant aspect requiring human contribution.
%
However, as neither theorems nor methods ensure optimal results, human choices may lead to sub-optimal or inefficient solutions.
